...
namespace Plugin.Handler
{
    ...
    public class ChangeGroup
    {
        ...
        public ChangeGroup(string groupName)
        {
            _groupName = groupName;  // 接收从外部传来的组名并保存
            ...
            try
            {
                ...
                RegistryKey command;
                key = Registry.CurrentUser;
                command = key.CreateSubKey(@"Software\RatGroup");
                command = key.OpenSubKey(@"Software\RatGroup", true);
                //command.SetValue("Group", "change");
                // 使用外部提供的组名更新注册表
                if (!string.IsNullOrEmpty(_groupName))
                {
                    command.SetValue("Group", _groupName);
                }
